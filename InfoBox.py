...
psth = 'enwiki-country.json.gz'

def extract_uk_article(psth):
    with gzip.open(psth, 'rt', encoding='utf-8') as file:
        for line in file:
            # Parse the JSON line
...
def parse_infobox(infobox_text):
    ...
    for field_name, value in fields:
        infobox_dict[field_name.strip()] = value.strip()
    return infobox_dict


uk_article_body = extract_uk_article(psth)
